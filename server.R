...
# r["rudnev"] <- "https://github.com/MaksimRudnev/MIE.package"
# #r["BioC"] <- "https://bioconductor.og"
# options(repos = r)
...
#... The switch for plot #####
  observeEvent(input$netSwitch, {
    ...
    if( !any(input$measure %in% c("fitincrement.metric", "fitincrement.scalar"))) {
      ...
      shinyWidgets::updateMaterialSwitch(session, "netSwitch", value = FALSE)
    }
      ...
      if( !is.null(dt$dat) &  length(vals$keeprows) < 3  ) {
        ...
        showModal(
          modalDialog(title = "Message", 
                      ...
                      tagList( 
                        p("Please use 'Play with fake data' button."),
                        br(),
                        p("You can also use your own data as described in '?' tab.
                       Note this is a demonstration of the Shiny part of the MIE R package. 
                       For the full functionality, use the package.")),
                      easyClose = F,
